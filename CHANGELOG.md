...
|---|---|---|
| 1.3.0 | 2026-09-18 | Full d'estil comú de les 12 figures gràfiques del capítol 4 (`analysis/scripts/estil_figures.py`, importat per `07_figures_cap4.py`): mides finals d'impressió, tipografia, paleta de tipologies segura per a daltonisme, grisos i trama per a tot el que no és tipologia, noms del vocabulari i coma decimal; eixida SVG amb text editable i PNG a 600 ppp, reproduïble byte a byte. Cap canvi de dades ni de xifres, llevat de la FIG-4.4b, que passa a dir «18 estructures al 100 %» perquè és el que donen les mètriques del pas 06 (la memòria deia 17). |
| 1.2.0 | 2026-09-17 | Taules de corpus i riquesa per tipologia per al capítol 4 de la memòria (`analysis/scripts/05_taules_cap4.R`, `resultats_pas3.txt` i les taules corresponents); mètriques de murs i de portals per EA (`06_metriques_cap4.py`) i generador de les figures gràfiques del capítol 4 (`07_figures_cap4.py`) amb les seues eixides en SVG; extracció del pas 00 també en Python (`00_extract_data.py`). Cap canvi d'esquema ni de dades. |
