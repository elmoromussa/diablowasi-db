...
' ================================================================
'  CHACHAPOYA - IMPORT CSV INTO A FRESHLY BUILT v17
'  Author: Esteve Ribera Torro | TFM Arqueologia UA
'  Spec: DELTA_v16_v17.md, section 10
'
'  RUN Sub ImportAll() ON AN EMPTY v17 DATABASE, i.e. one on which
'  chachapoya_DB_v17.bas -> BuildDB() has just run and into which
...
'  no matching field in v17 is not an error to abort on - it is
'  exactly what a withdrawn field looks like from here. It is
'  reported once, by name, so that a column skipped by ACCIDENT
'  is visible too.
'
'  TWO THINGS THIS IMPORTER DOES THAT THE LAST ONE DID NOT.
'
'  1. IT NORMALISES THE CONNECTION PAIRS, and inverts the
'  chronology when it does. In v16 the direction lived in the
'  position of A and B, which is why the schema had to carry an
'  instruction never to swap them; in v17 the pair is ordered by
'  id and a unique index makes the duplicate impossible. Ordering
'  without inverting would flip the meaning of every directional
'  row IN SILENCE, which is precisely the class of error this
'  whole delta was written to remove.
'  It also DEDUPLICATES, because normalisation makes the corpus's
'  two rows collide - they are the same connection entered from
'  each end. The first is kept and the second reported. If the
'  two disagree on type, chronology or confidence, that is said
'  out loud rather than resolved by whichever happened to be
'  first in the file.
'
'  2b. THE TYPOLOGY 'EA-PLA-R Ledge Platform' WAS RENAMED to
'  'EA-TER Ledge Terrace' in v17, so a CSV carrying the old name
'  will not resolve. This one IS remapped automatically, because
'  it is a rename and not a re-reading: the definition did not
'  change, only the word, which was being shared with element H
'  and with typology EA-PLA-V.
'
'  2. IT REFUSES TO GUESS A RETIRED DECORATION TYPE. Three
'  entries left L_DEC_TYPE in v17 - Frieze / Greca, Triangular
'  motif and Decapitation scene - all unused in the corpus. If a
'  row turns up carrying one, the type is left NULL and named,
'  rather than mapped to whatever seems closest: Decapitation
'  scene belongs to RA Anthropomorphic and Frieze / Greca to ND,
'  but those are readings, and a reading made by an import
'  script is a reading nobody made.
'
...
Sub ImportAll()
    ...
    If DCount("*", "T_STRUCTURES") > 0 Then
        MsgBox "T_STRUCTURES is not empty. Run this only on a freshly built v17." & vbCrLf & vbCrLf & "Build a blank database with chachapoya_DB_v17.bas -> BuildDB() and import into that.", vbCritical, "Import aborted"
        Exit Sub
    End If
...
Private Function ImportStructures(db As DAO.Database) As Long
    ...
    For i = 0 To mNRow - 1
        ...
        Dim tnm As String
        tnm = CV(r, "Typology")
        If tnm = "EA-PLA-R Ledge Platform" Then tnm = "EA-TER Ledge Terrace"
        tid = IdOf(db, "L_TYPOLOGY", "Name", tnm)
...
Private Function ImportDecorations(db As DAO.Database) As Long
    ...
    For i = 0 To mNRow - 1
        ...
        If did = 0 And Len(Trim$(CV(r, "Dec_Type"))) > 0 Then
            If IsRetiredDecType(CV(r, "Dec_Type")) Then
                Debug.Print "  *** T_DECORATIONS row " & (i + 2) & ": type '" & CV(r, "Dec_Type") & "' was retired in v17 - left blank for review, NOT reassigned"
                nRet = nRet + 1
            End If
        End If
        If sid = 0 Then
        ...
        Else
            ...
            ' The four segments and the outline geometry come from
            ' the CSV, where they are blank unless the reviewer
            ' filled them against the photograph. Blank -> Null,
            ' never 0: a 0 would assert that the segment carries no
            ' pigment, and nobody has looked yet.
            ' OLD_Position_Relative is ignored, like every OLD_
            ' column. It cannot be mapped mechanically: 'Both' was
            ' being used in the corpus to mean an inverted U, so
            ' deriving the segments from it would assert an absent
            ' crown on exactly the rows where the crown is there.
            rs!Span_Left = NV(CV(r, "Span_Left"))
            rs!Span_Above = NV(CV(r, "Span_Above"))
            rs!Span_Right = NV(CV(r, "Span_Right"))
            rs!Span_Below = NV(CV(r, "Span_Below"))
            rs!Outline_Geometry = NV(CV(r, "Outline_Geometry"))
    ...
    Debug.Print "   rows carrying a decoration type retired in v17: " & nRet
    ImportDecorations = n
End Function
...
' ================================================================
'  T_CONNECTIONS - NORMALISED, INVERTED AND DEDUPLICATED
' ================================================================
Private Function ImportConnections(db As DAO.Database) As Long
    ...
    For i = 0 To mNRow - 1
        ...
        ct = CV(r, "Connection_Type")
        cf = CV(r, "Confidence")
        ' The v16 wording maps one-to-one onto the v17 one: both
        ' name the slot holding the earlier structure.
        cr = MapChrono(CV(r, "Chrono_Relation"))
        If a = 0 Or b = 0 Then
            Debug.Print "  *** T_CONNECTIONS row " & (i + 2) & ": " & CV(r, "Code_A") & " / " & CV(r, "Code_B") & " - one end not found, skipped"
            bad = bad + 1
        ElseIf a = b Then
            Debug.Print "  *** T_CONNECTIONS row " & (i + 2) & ": both ends are the same structure - skipped"
            bad = bad + 1
        Else
            ' NORMALISE, AND INVERT WITH IT. Ordering the pair
            ' without turning the chronology round would flip the
            ' meaning of every directional row in silence.
            If a > b Then
                Dim sw As Long
                sw = a: a = b: b = sw
                If cr = "A is earlier" Then
                    cr = "B is earlier"
                ElseIf cr = "B is earlier" Then
                    cr = "A is earlier"
                End If
                nSwap = nSwap + 1
            End If
            ' DEDUPLICATE. After normalisation the same connection
            ' entered from both ends collides, and the unique index
            ' would stop the import dead. Keep the first, report
            ' the second - and if they disagree, say so, because
            ' picking one in silence is picking one at random.
            Dim dupq As String
            dupq = "SELECT * FROM T_CONNECTIONS WHERE ID_Struct_A=" & a & " AND ID_Struct_B=" & b
            Dim rd As DAO.Recordset
            Set rd = db.OpenRecordset(dupq, dbOpenSnapshot)
            If Not rd.EOF Then
                Debug.Print "  [DUP] T_CONNECTIONS row " & (i + 2) & ": " & CV(r, "Code_A") & " / " & CV(r, "Code_B") & " already present after normalisation - not written"
                If Nz(rd!Connection_Type, "") <> Nz(ct, "") Then Debug.Print "        DISAGREES on type: kept '" & Nz(rd!Connection_Type, "") & "', dropped '" & ct & "'"
                If Nz(rd!Chrono_Relation, "") <> Nz(cr, "") Then Debug.Print "        DISAGREES on chronology: kept '" & Nz(rd!Chrono_Relation, "") & "', dropped '" & cr & "'"
                If Nz(rd!Confidence, "") <> Nz(cf, "") Then Debug.Print "        DISAGREES on confidence: kept '" & Nz(rd!Confidence, "") & "', dropped '" & cf & "'"
                nDup = nDup + 1
            Else
                rs.AddNew
                rs!ID_Struct_A = a
                rs!ID_Struct_B = b
                rs!Connection_Type = NV(ct)
                rs!Chrono_Relation = NV(cr)
                rs!Confidence = NV(cf)
                rs!Notes = NV(CV(r, "Notes"))
                rs.Update
                n = n + 1
    ...
    Debug.Print "   pairs reordered (chronology inverted with them): " & nSwap
    Debug.Print "   duplicate pairs dropped: " & nDup
    ImportConnections = n
End Function

' The v16 wording and the v17 wording say the same thing about
' the same slot, so this is a rename and not a re-reading.
' Anything unrecognised travels unchanged and shows up in
' QRY_16, which is better than being quietly discarded.
Private Function MapChrono(v As String) As String
    Dim t As String
    t = Trim$(v)
    If t = "A earlier than B" Then
        MapChrono = "A is earlier"
    ElseIf t = "B earlier than A" Then
        MapChrono = "B is earlier"
    Else
        MapChrono = t
    End If
End Function

' The three types retired in v17. Named, never remapped: the
' closest surviving entry is a reading, and a reading made by an
' import script is a reading nobody made.
Private Function IsRetiredDecType(v As String) As Boolean
    Dim t As String
    t = Trim$(v)
    IsRetiredDecType = (t = "Frieze / Greca")
    IsRetiredDecType = IsRetiredDecType Or (t = "Triangular motif")
    IsRetiredDecType = IsRetiredDecType Or (t = "Decapitation scene")
End Function
