...
L'esquema segueix els principis de la tercera forma normal (3FN) i inclou 18 taules, una taula principal (T_STRUCTURES) amb 116 camps organitzats en 18 categories temàtiques, 18 relacions, 13 consultes SQL i un formulari d'entrada de dades amb 11 pestanyes i interfície en valencià. Un dels avanços metodològics centrals és la normalització d'un vocabulari arquitectònic bilingüe (valencià/anglés) de 24 elements (A-X), derivat de l'anàlisi fotogramètrica 3D de les estructures de Diablo Wasi, que permet el registre sistemàtic dels sistemes constructius i la seua anàlisi comparativa entre jaciments i sectors. La versió actual de la BD (v3) amplia l'esquema inicial amb deu camps nous en quatre grups temàtics (detall del suport geològic, qualitat de la maçoneria, paisatge i orientació, fases constructives) i tres consultes noves, incloent-hi l'exportació del vector A-X per a l'anàlisi de patrons constructius (coocurrència, clúster jeràrquic i autocorrelació espacial).
...
| H02 | Geologia com a factor determinant | ID_Support, Support_Width_cm, Support_Depth_cm, Support_Morphology, Support_Modified, Natural_Roof, Altitude_masl, Approx_Height_m, Base_Level, Tie_Walls (QRY_12) |
...
|---|---|---|
| Principal | T_STRUCTURES | Fitxa completa (116 camps). Font principal per a l'anàlisi estadística. |
...
*Taula 3. Les 18 taules de la base de dades amb la seua funció.*

## **5. Variables arqueològiques de T_STRUCTURES (116 camps)**

La taula T_STRUCTURES concentra 116 camps en 18 categories temàtiques (les 14 originals més els quatre grups incorporats en la revisió v2). La taula següent resumeix les categories i el nombre de camps per categoria:
...
| Sistemes N1 (vocab. J-W) | Access_Opening (P), Sill (N), Jambs (O), Lateral_Walls (V), Rear_Wall (W), Rear_Wall_Built, Recessed_Portal, Structural_Pilasters (K), Cornice_Material | 9 |
...
| Decoració (booleans) | Dec_Square_Niche, Dec_Relief_T/T_Inv/L/L_Inv, Dec_Zigzag, Dec_Stepped, Dec_Frieze, Rock_Art, RA_Anthropomorphic/Zoomorphic/Geometric/Abstract/Decap_Scene | 14 |
...
*Taula 4. Categories temàtiques de T_STRUCTURES amb nombre de camps per categoria (total: 116).*
...
La BD inclou 13 consultes SQL predissenyades que cobreixen les principals anàlisis del TFM:
...
| QRY_13_AX_Pattern_Export | NOVA v3. Vector A-X net per estructura per a l'anàlisi de patrons en R. | H01, H04, H05 |

*Taula 5. Les 13 consultes SQL predissenyades i la seua vinculació amb els objectius i hipòtesis del TFM.*

### **9.1. Estratègia d'anàlisi de patrons constructius (QRY_13)**

L'atomització de les estructures en el vocabulari A-X converteix cada estructura en un vector binari de presència/absència d'elements constructius, complementat amb Masonry_Quality, Masonry_Type, la morfologia del suport i les coordenades UTM. La consulta QRY_13_AX_Pattern_Export exporta aquest vector de forma neta (LEFT JOIN amb L_TYPOLOGY per a no excloure registres parcials), i habilita quatre famílies d'anàlisi en R:
...
El formulari F_STRUCTURES (chachapoya_Form_v2_val.bas) centralitza l'entrada de dades amb 11 pestanyes temàtiques, dos subformularis vinculats i tots els camps FK configurats com a ComboBox. Tota la interfície és en valencià; tots els valors emmagatzemats són en anglés (secció 2.1).
...
| 2. Arq. | Morfologia: N_Floors, Floor_Plan, dimensions, Access_Orientation, Lintel, Natural_Roof, Buttresses, Interlevel_Cornice, Wooden_Stakes + Facade_Orientation, Visibility_Valley, Masonry_Quality, Masonry_Type, Construction_Phases, Phase_Evidence |
...
*Taula 6. Estructura del formulari F_STRUCTURES amb 11 pestanyes temàtiques.*
...
- chachapoya_DB_v3.bas - Sub BuildDB(): crea les 18 taules en ordre de dependència, pobla els 12 lookups, estableix les 18 relacions i genera les 13 consultes. S'executa sobre una BD en blanc. És idempotent: elimina i regenera les consultes existents i comprova l'existència de taules abans de crear-les.
- chachapoya_Form_v2_val.bas - Sub BuildForm(): crea els subformularis F_DECORATIONS i F_ARCH_FEATURES, construeix F_STRUCTURES amb 11 pestanyes, configura tots els ComboBox per ControlSource i estableix les captions DAO. S'executa després de BuildDB().
