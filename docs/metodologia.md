...
| H02 (nota v26: `Support_Modified` hi participa amb **estatut documental** — els 6 positius es discuteixen qualitativament com a traça del treball de la roca, el camp queda fora de la matriu analítica pel 9 dominant, 92/106) | Geologia com a factor determinant | ID_Support + Support_Mode (v13: diàclasi vs junta d'estratificació), ID_Support_Secondary, Support_Width_cm, Support_Depth_cm, Support_Modified, Altitude_masl, Height_Above_Base_m, Base_Level, Tie_Walls, Rear_Closure_Type, Chamber_Roof_Type (QRY_12) |
...
**Primera: la compartició d'element es converteix en patró — i el patró, en v26, demostra el seu criteri de jubilació.** Quan la parella de muntants (`Jamb_Fabric`, v25) va completar el seu domini sobre les esqueixades, el qualificador va esdevindre derivable al 100% (22/22 sobre el corpus tancat) i **es jubila en v26**: el cas EA02, on parella i qualificador es contradeien, havia demostrat la patologia de dos-llocs-per-a-un-fet. La classe de camp sobreviu amb un membre (`Sill_Coincides_Cornice`), i el mecanisme queda com a criteri metodològic: **un qualificador viu mentre diu una cosa que cap altre camp diu; quan esdevé derivable, es jubila i la derivació es documenta**. Descripció històrica del membre jubilat: `Jamb_Fabric_Reveal` registra la posició del brancal resolta per la fàbrica mateixa — una cara terminal acabada i deliberada (*masonry reveal*) — i replica exactament el mecanisme de `Sill_Coincides_Cornice`: domini 0/1/9 amb zero de farciment, finestra d'aplicabilitat vigilada per regla (la 55), i exempció de la regla de coherència que altrament llegiria el cas legítim com a registre a mig entrar (la branca de portal de la regla 3). El que en v18 era una excepció puntual és ara una **classe de camp**: el qualificador que diu *com* es va resoldre una posició quan cap element diferenciat la resol. La conseqüència d'interfície és deliberada: aquests camps responen una pregunta, no declaren una presència, i per això porten etiquetes pròpies (*No / Sí / No observable*) sobre els mateixos valors emmagatzemats — dos vocabularis d'etiqueta per a un únic domini, documentats perquè no semblen arbitraris.
...
Es manté el criteri fixat: N0/N1/Superior són **categories funcionals, no un sistema de numeració** (mai N2, N3), i un cos pertany a N1 si conté o contenia una obertura d'accés. **La v26 hi afig la reformulació documental base/superestructura (Guengerich 2014)**: N0 = *base* (massa de suport sense espai interior accessible), N1 = *superestructura* (allò que la base sosté — típicament una cambra, també un aparell obert de coronament o circulació). El límit que la va provocar és empíric: EA07, EA08 i EA72 porten ràfec, plataforma o coronament sobre un cos basal sense cap cambra — sota la lectura base/cambra eren excepcions gestionades per doctrina («ràfec sobre cos basal»); sota base/superestructura són regulars. I el pol invers també existeix: EA-PLA-V (EA20) és superestructura sense base, amb el penya-segat fent de base com el nínxol fa de cambra a EA16. Que la roca puga substituir **qualsevol dels dos termes de la parella de Guengerich** és exactament l'argument del cingle com a participant estructural — i que l'arquitectura funerària de cingle compartisca la gramàtica base/superestructura de l'arquitectura residencial chachapoya alimenta directament la hipòtesi de seqüència constructiva compartida. Cap camp no es toca: el corpus no conté cap cos de superestructura comptable que no siga cambra, i sense cas confirmat no es crea res. El recompte viu als camps N_Basal_Bodies i N_Chamber_Bodies, i la matriu es manté plana (una fila per estructura). La novetat v11 és el destí de l'evidència de cossos perduts: l'antic camp Lost_Body_Evidence desapareix i la seua informació esdevé una fila de T_LOST_ELEMENTS amb abast *Body*, que cobreix el vocabulari sencer del cos desaparegut — el cas diagnòstic continuen sent les bandes verticals de pigment sobre la roca nua, el fantasma d'un parament que la pintura ha sobreviscut. La conseqüència analítica és ara més fina: els elements del cos desaparegut ja no s'han de marcar 9 en bloc, sinó que poden marcar-se 3 amb l'evidència que ho justifica, cosa que els reincorpora al recompte de *què es va construir*.
...
## Nota de versió v25f — El graf abans que el grup

**Per què arestes i no clusters.** La primera temptació davant d'un penya-segat organitzat en estrats era la natural: crear un grup per estrat i assignar-hi les estructures. El model va caure per les seues pròpies dades abans d'escriure's: dues estructures pertanyien alhora a dos estrats de la llista de camp (encreuaments de trams), i vint-i-dues participaven simultàniament en una cadena horitzontal i en una de proximitat vertical — i `ID_Group` només n'admet un. La solució no va ser afegir una taula de pertinences N:M sinó adonar-se que **el cluster no és un fet: és una anàlisi**. El fet observable és el parell — aquestes dues estructures comparteixen estrat, aquestes dues es toquen, aquestes dues es miren en vertical — i el parell encadenat és exactament una aresta de graf. Els conjunts que la vista reconeix (l'estrat sencer, l'alineament) es deriven a R com a components connexos, amb la família d'arestes com a filtre: el mateix registre serveix la lectura geològica (components de `Shared support` ∪ juntes), la constructiva (juntes i superposicions, dirigides per estratigrafia) i la de circulació aèria (H03, sobre les associacions de verticalitat). Registrar el parell i derivar el conjunt és la mateixa disciplina que registrar l'element i derivar el sistema: la base guarda observacions, no interpretacions.

**L'ordre de precedència com a extensió d'un criteri existent.** Quan un parell admetia dues lectures (adossades *i* al mateix estrat; adossades *i* pròximes), el delta generalitza el criteri que l'esquema ja tenia per a un cas («la junta mana sobre el suport»): guanya la relació més específica — contacte > proximitat > estrat compartit — perquè la més específica implica les altres o les fa derivables, i és l'única que pot portar direcció cronològica. Deu parells d'estrat van quedar subsumits per la seua junta, un quedà mediat per una estructura interposada descoberta durant la mateixa campanya (EA77, arranc murari entre EA04 i EA05) i un altre es reclassificà com a associació horitzontal quan la descripció de camp va precisar que l'estrat no era estricte: cap pèrdua, la co-pertinença es recupera del component.

**La verificació geomètrica com a peatge d'entrada.** Cap cadena no s'ha entrat sense passar pel filtre de coordenades: els dotze estrats mostren desnivells consecutius mètrics (compatibles amb el cabussament suau del paquet calcari) i cap inversió d'ordre al llarg del front; les setze cadenes de proximitat mostren separació vertical franca amb un únic cas límit, documentat a la seua aresta. La passada inversa — veïns a menys de 3.5 m sense cap aresta — va resultar la més productiva: va caçar una adossada impossible (11.8 m entre marcadors amb precisió centimètrica), una omissió, i va generar una llista de candidats que l'arqueòleg va triar un per un, acceptant-ne sis i descartant-ne quatre amb criteri explícit. És la divisió del treball correcta: la geometria proposa, l'observació disposa. I és la mateixa lliçó de la nota v25e aplicada en positiu: els rangs es comproven **abans** d'escriure, no després de sospitar.

## Nota de versió v25e — Epistemologia de la parella incompleta i l'incident del punt decimal

**El domini NObs com a decisió epistèmica.** La primera campanya de classificació de muntants va topar amb sis portals d'un sol costat llegible, i el registre els va absorbir amb la pitjor eina disponible: duplicar el membre observat. La solució v25e — tres valors de parella amb segon membre no observable — és deliberadament epistèmica i no tipològica: no afig cap combinació constructiva al món, afig la capacitat de dir «això és el que he vist i això és el que no puc veure» sense destruir ni l'una ni l'altra meitat. El criteri de «cap categoria sense cas» es satisfà sense reserva (els tres valors tenen cas), i la redefinició d'ND que l'acompanya (cap muntant amb resolució *determinable*) naix d'un cas concret, DW-S04-EA12, que va ensenyar la diferència entre *no observable* i *indeterminat*: hi ha llosa, però sense la part alta del muntant no es pot saber si era llosa completa o composta. Que el fet positiu (la llosa existeix) visca a l'element O i no a la parella és el principi de no registrar cap fet dues vegades treballant a favor de la incertesa: cada capa carrega exactament la seua part del que se sap.

**L'incident del punt decimal com a lliçó de mètode.** La revisió prèvia al paquet v25e va destapar que la línia d'escriptura d'`ApplyMetricReview` convertia el text de la proposta amb una funció sensible al locale, i sota configuració regional de coma decimal el punt es llegia com a separador de milers: 125 valors mètrics van entrar a la base multiplicats per 100 mentre l'acta en pantalla mostrava el valor correcte. Tres coses en queden per al mètode. Primera: la **frontera text→número és un lloc del codi**, no una operació innòcua — el codebase adopta com a norma `Val()` (interpretació fixa del punt) i reserva `CDbl` per a valors ja numèrics. Segona: la **verificació de rangs sobre la base viva** (les dimensions d'una cambra chachapoya no fan 181 metres) hauria caçat l'error el mateix dia; s'incorpora com a pas de revisió després de cada aplicació en bloc. Tercera, i la més important: la reparació no va necessitar ni tocar els CSV ni re-executar el pipeline, perquè `T_METRIC_REVIEW` conserva la proposta literal de cada firma — **el registre d'auditoria com a font de reparació**. El disseny que es va justificar en v25 amb «d'ací a tres mesos se sabrà per què cada camp val el que val» ha demostrat que també respon la pregunta simètrica: què ha de valdre un camp quan el mecanisme d'escriptura ha fallat. La traçabilitat no és un cost burocràtic del mètode: és la seua pòlissa.
