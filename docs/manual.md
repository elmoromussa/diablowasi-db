...
> **Què ha canviat en v25f, en una ullada.** El mapa de relacions ha entrat sencer. `T_CONNECTIONS` passa de 5 a 77 arestes en cinc famílies del domini de sempre: **Suport compartit** (mateix estrat geològic, la distància no importa), **Junta vertical adossada**, **Superposició**, **Context natural associat** (amb la regla: el context natural sempre és l'anterior) i **Associació de verticalitat** (proximitat vertical o diagonal sense contacte). Les cadenes s'han entrat com a parells consecutius en l'ordre físic (verificat per coordenades), i quan un parell tenia dues lectures ha guanyat la més específica: contacte > proximitat > estrat. Els *clusters* (un estrat sencer, un alineament) **no es guarden**: es deriven a R com a components connexos del graf — per això `T_GROUPS` continua buida i `ID_Group` també, i està bé que així siga. La paternitat s'ha ampliat: quatre pares-context (la cova EA42, el diedre EA76 — ara CAV, com mana la doctrina de tipologia —, i les coves DW-S05-EA02 i DW-S06-EA04) amb nou filles noves. Cronologies estratigràfiques firmades: EA71→EA22, EA40→EA72, EA06→EA62, i els dos nínxols naturals anteriors a les seues estructures. Al formulari no canvia res; reobri `QRY_14_Connections_Edges` per a veure el graf i `QRY_16_Validation_Check` per al verd.
>
> **Què ha canviat en v25e, en una ullada.** Tres coses. (1) El combo de **Muntants del portal** té tres valors nous per a portals amb un sol costat llegible (Llosa/Composta/Fàbrica **+ no observable**) i ND queda per a «cap muntant amb resolució determinable»; la regla R69 s'ha esmenat en conseqüència (Fàbrica + no observable no dispara mai). (2) S'ha reparat un bug de locale que havia escrit ×100 els valors mètrics aplicats pel pipeline (125 valors en 6 camps): si entre l'estrena del pipeline i esta reparació vas exportar o consultar Length/Width/Height, àrees o obertures, **reexporta-ho** — els valors ara són correctes en metres. La reparació queda estampada a `Notes` de cada fila de `T_METRIC_REVIEW`. (3) Sis parelles de muntants entrades duplicant l'únic membre observat s'han reclassificat per cirurgia firmada (acta al DELTA v25d→v25e), i el JFR de DW-S02-EA02 ha passat a 9 amb nota. Res d'això et demana acció al formulari: només reobrir `QRY_16_Validation_Check` i comprovar el verd.
>
> **Què ha canviat en v25, en una ullada.** La worklist té tres fonts noves. Les files **«Revisio metrica»** són discrepàncies que el pipeline `CheckMetrics` ha trobat entre els CSV de mètriques i la BD: NO es resolen al formulari sinó **firmant `Decision` al full de dades de `T_METRIC_REVIEW`** (Accept = el CSV mana i `ApplyMetricReview` ho escriurà; Keep DB = la BD tenia raó, documenta-ho a Notes; Investigate = cal mirar el model o el camp). Cap valor s'escriu mai sense la teua firma. Les files **«Classificacio»** són els dos camps nous: **Muntants del portal** (11.Sist, fent parella amb «Fàbrica fa de brancal» — cada costat de l'obertura resol com a llosa / composta (llosa baix + fàbrica travada damunt) / fàbrica, i el camp registra la parella sencera: Llosa + llosa, Llosa + composta, Llosa + fàbrica, Composta + composta, Composta + fàbrica, Fàbrica + fàbrica; ordre canònic el més fort primer, sense esquerra/dreta. S'activa amb el portal, tinga brancals o no. Recorda: la trava mana, una falca d'anivellament no fa «composta»; i des de **v25e** les parelles amb un membre no observable: **Llosa + no observable, Composta + no observable, Fàbrica + no observable** — quan només un costat és llegible, tria la parella NObs i deixa de duplicar el membre observat. **ND** vol dir des d'ara «cap muntant amb resolució *determinable*»: no observable o indeterminat — si es conserva llosa però no pots saber si era llosa o composta (cas EA12: sense dintell ni res per damunt), el fet positiu ja el porta l'element O i la parella és ND) i **Partició de nínxol** (1.Id, davall de la geologia — la llosa horitzontal que parteix el foraet en dos; només s'activa en NIX). I si mai marques 3 en un element o Attested lost en la partició, l'evidència va a 12.Extra com sempre.
>
...
| Context funerari natural | — (2.Arq oberta en v22, amb «Maçoneria present» de porter) |
...
**La maçoneria no depén dels murs — però comença per la declaració (v21).** El bloc de maçoneria l'encapçala ara **«Maçoneria present»** (0/1/9): *hi ha fàbrica de maçoneria en aquest registre?* Una terrassa sense cap mur construït en té igualment — filades amb aparell, qualitat i format de pedra (13 registres del corpus ho demostren) — i una **mènsula aïllada no en té cap**: per això la pregunta és teua i no de la tipologia. **Amb 0 o 9 tot el bloc es tanca sol** (format, treball, qualitat, aparell, morter, ripio, notes i fàbrica); amb 1 (o encara buit) s'obri i emplenes el detall. Si un detall queda escrit sota una declaració 0 o 9, la regla 61 t'ho recordarà. **v22: això val també per als contextos naturals** — la pestanya s'obri per a la classe, i la primera pregunta d'una cova o un nínxol és exactament esta: *hi ha fàbrica?* Amb un 0 has acabat la pestanya; amb un 1 (bigues, murets, pis de lloses dins de la cavitat) emplenes el detall com en qualsevol estructura. **Els panells no**: el seu 0 s'ompli sol en triar la tipologia.
...
**Maçoneria.** Primer la declaració, després el detall. Vegeu el **nus 4**. I si la fàbrica no és igual per tot arreu, el **nus 9**.
...
**8.Cron (v22): només C14.** Els segles d'inici i fi **només s'obrin amb la casella C14 marcada** — el projecte data exclusivament per radiocarboni, així que un segle sense C14 darrere no té font (regla 64). Marca el C14, entra la datació al subformulari, i llavors sí, els segles.

**9.Metr (v22): primer el porter.** La pestanya comença amb **«Dades mètriques disponibles»** (0/1/9): amb 0 o 9 tot el bloc dimensional es tanca sol i has acabat; amb 1 (o encara buit) emplenes el que tingues. El **9** vol dir *no mesurable mai* — sense accés i sense cobertura de model —, distint del buit (*encara no decidit*). Les **coordenades i les notes queden sempre obertes**: una estructura sense res mesurable continua tenint posició. Este judici **no apareix a cap llista de treball** a propòsit: es farà en la segona passada, quan el flux d'extracció del model estiga en marxa.
...
## Nus 12 — La vora superior d'una estructura sense cambra (v21)

Una terrassa de dues o tres filades rematada per lloses: què és, allò de dalt? **Una sola pregunta real: hi ha remat?**

> **Hi ha remat?** Tractament terminal diferenciat — les lloses que coronen la massa → **Coronament (R) = 1 o 2**, i el seu format (*filera en voladís*, normalment). Si no n'hi ha, **R = 0**; si és indecidible, **9 + nota**.

**I el pla d'ús? No s'escriu enlloc, i és a propòsit (v21).** Una terrassa **ja respon la pregunta d'ús per la seua pròpia tipologia**: `EA-TER` *és* una massa construïda amb el cap utilitzable — escriure-ho una segona vegada en un camp no afegiria cap observació. La **superfície de plataforma (Z) queda reservada a la superfície del sistema VOLAT**: les lloses recolzades sobre les mènsules (E) o sobre la biga transversal (F). **El cap de la massa mai és Z i mai obri el sistema plataforma.** Si obris el sistema amb E, F i G confirmats a 0, la regla 62 t'ho recordarà — i el cas dels **suports irresolubles** (E/F/G a 9 amb superfície visible) continua sent plataforma, com sempre: la reserva tanca el cap de massa, no el dubte.
