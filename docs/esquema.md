...
`src/chachapoya_DB.bas` + `src/chachapoya_Form.bas` (v25g: `BuildDB()` + `BuildForm()`; `tools/chachapoya_Worklist.bas` com a navegador; `tools/chachapoya_CheckMetrics.bas` com a pipeline de coherència mètriques↔BD)

Sub BuildDB() + Sub BuildForm() | Microsoft Access JET SQL | Esteve Ribera-Torró

*Document actualitzat segons el codi v25g (setembre 2026). El registre de canvis de cada versió és al `CHANGELOG.md` de l'arrel i als deltes de `docs/deltas/`; la secció 1 resumeix les iteracions v10 a v20 i el §3.6 formalitza la doctrina de capes del model de relacions.*
