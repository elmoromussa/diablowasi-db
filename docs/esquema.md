...
`src/chachapoya_DB.bas` + `src/chachapoya_Form.bas` (v26: `BuildDB()` + `BuildForm()`; `tools/chachapoya_Worklist.bas` com a navegador; `tools/chachapoya_CheckMetrics.bas` com a pipeline de coherència mètriques↔BD)

Sub BuildDB() + Sub BuildForm() | Microsoft Access JET SQL | Esteve Ribera-Torró

*Document actualitzat segons el codi v26 (setembre 2026). El delta v25g→v26 tanca la sèrie abans del depòsit: **jubilació de `Jamb_Fabric_Reveal`** (derivable al 100% de la parella de muntants — 22/22 sobre el corpus tancat; regla de derivació: 1 si la parella conté *Fabric*, 9 si sense *Fabric* conté *NObs* o és *ND*, 0 en la resta i en la parella buida; regla 3 i regla 55 reformulades sobre l'expressió derivada, branca JFR de QRY_24 retirada); **reformulació documental base/superestructura** (Guengerich 2014) del marc N0/N1 — cap camp tocat, definicions reescrites (§2.4, §8bis): N0 = base (massa de suport sense espai interior accessible), N1 = superestructura (allò que la base sosté — típicament cambra, també aparell obert de coronament o circulació: EA07/EA08/EA72), amb EA-PLA-V com a pol invers (superestructura sense base, el penya-segat fent de base com el nínxol fa de cambra a EA16); **estatut documental de `Support_Modified`** (fora de la matriu analítica); **doctrina de la família posicional del voladís** (G plataforma / I juntura / R-*Projecting course* coronament — un gest tècnic, tres posicions; *sub-cornice* de Guengerich Fig. 3 continua sense cas, destí `T_ARCH_FEATURES` si mai n'apareix un); i l'acta de dades de la sessió 2026-09-06 (24 valors en 15 files de `T_STRUCTURES` + 5 notes a `T_LOST_ELEMENTS`; vegeu DELTA_v25g_v26.md). Incorpora també les esmenes de regla v25g (R54 *Rock dihedral* a CAV; R66 exclou *Not applicable*; finestra v20 retirada). El registre de canvis de cada versió és al `CHANGELOG.md` de l'arrel i als deltes de `docs/deltas/`; la secció 1 resumeix les iteracions v10 a v20 i el §3.6 formalitza la doctrina de capes del model de relacions.*

**Migració in situ, no reconstrucció (v20 a v25).** `PatchV25()` afig `T_METRIC_REVIEW` (FK inline a `T_STRUCTURES`, captions DAO), `Jamb_Fabric` i `Niche_Partition` a `T_STRUCTURES`, i `Term_Lit`/`Lit_Source` a `L_ELEMENTS` amb les tres concordances (guardades: només ompli NULL); `RebuildQueriesV25()`, `BuildForm()` i `BuildWorklist()` fan la resta, i `CheckMetrics` pobla la revisió amb els CSV nets vigents. Els pedaços v20 a v24 (`tools/patches/`) segueixen el mateix mecanisme, descrit al delta de cada versió. `BuildDB()` **no s'ha d'executar mai** sobre una base amb dades: fa DROP de `T_DECORATIONS`, `T_LOST_ELEMENTS`, `T_CONNECTIONS` i `T_ARCH_FEATURES`.

**Construcció de zero, transferència en dos passos (referència v17).** La base v17 es construeix des de zero amb els scripts canònics i les dades hi arriben per `tools/migrations/chachapoya_EXPORT_v16.bas` → CSV inspeccionable i editable → `tools/migrations/chachapoya_IMPORT_v17.bas`. **Cap valor arriba sense haver pogut ser mirat**, i els camps el criteri dels quals ha canviat arriben NULL: un valor transferit sense revisió afirmaria un judici que ningú no ha fet sota el criteri nou. La precondició per a publicar qualsevol percentatge continua sent la mateixa — cada valor emmagatzemat, un judici deliberat.
...
| Valors per defecte | **Dos, deliberadament**: 0 als 21 camps d'element governats per un `Sys_*` (la regla 4 hi exigeix el zero de farciment) **i al qualificador de compartició (`Sill_Coincides_Cornice`)** (gating derivable, §2.5; el segon qualificador, `Jamb_Fabric_Reveal`, **es jubila en v26** — derivable de la parella de muntants); **NULL a la resta** — buit = no avaluat encara, 9 = avaluat i no examinable, 0 = avaluat i absent |
...
| **Camp** | **Tipus** | **Descripció** |
| --- | --- | --- |
| N_Basal_Bodies | INTEGER | Masses superposades **sense obertura** (nivell N0). **v26 (reformulació documental, Guengerich 2014): N0 = BASE** — massa constructiva de suport sense espai interior accessible |
| N_Chamber_Bodies | INTEGER | Cambres funeràries superposades (nivell N1). Criteri: un cos és N1 si conté (o contenia) obertura d'accés. **v26: N1 = SUPERESTRUCTURA** — allò que la base sosté: típicament una cambra, però també un aparell obert de coronament o circulació (EA07/EA08/EA72: ràfec, plataforma i coronament sobre cos basal sense cambra — la doctrina «ràfec sobre cos basal» passa d'excepció a teorema del marc). El comptador conserva nom i criteri: al corpus no existeix cap *cos* de superestructura comptable que no siga cambra (allà la superestructura s'expressa en elements U/R/Z, no en cossos). Pol invers: EA-PLA-V (EA20), superestructura **sense** base — el penya-segat fa de base com el nínxol fa de cambra a EA16 (`Sys_Chamber` NA / `Sys_Base` NA: la roca pot substituir qualsevol dels dos termes de la parella) |
...
| Support_Modified | BYTE | 0/1/9. Modificació antròpica del suport natural. Sovint oculta darrere de la maçoneria. **v26 ESTATUT: camp DOCUMENTAL, no analític.** El 9 dominant (92/106) és la doctrina funcionant: a les construïdes la fàbrica amaga el suport i el dubte sempre hi és (conversió 0→9 de l'acta v25g); els 0 sobreviuen només a contextos naturals (NIX/CAV, verificat 8/8) i els 6 positius — EA15, EA31, S04-EA12/13/14, S04-EA20 (els encaixos de mènsula: el penya-segat retallat per a rebre l'estructura) — són la traça directa del treball de la roca (OE4). **Exclòs de la matriu de co-ocurrència i del clustering**; discussió qualitativa |
...
*`Interbody_Cornice_Material` **es retira en v16**. No perquè de fet les cornises siguen sempre de pedra sinó perquè no podrien no ser-ho: el valor *Wooden beams* no descrivia una cornisa de fusta sinó **una plataforma mal classificada**. T i U ja porten «SEMPRE PEDRA» dins de la definició del vocabulari i no tenen camp de material; la cornisa era l'excepció incoherent. El guany no és estalviar un camp sinó que **una descripció feble es converteix en un test d'identificació** — el que fa falta per a discriminar G de I (8bis.7, regla 39). **v26: el test guanya la segona pota — la FUNCIÓ.** Unes filades que *semblen* cornisa intranivell però *treballen* de suport del tauler volat són G (cas documentat: EA01, filades de la plataforma); I marca la juntura entre cossos, G sosté tauler. La família posicional completa del voladís queda doctrina: **G plataforma / I juntura / R-Projecting course coronament** — un gest tècnic, tres feines (a EA07 la seqüència F→G→H es llig físicament: la biga transversal sosté les filades, les filades rematen la plataforma). La posició *sub-cornice* (Guengerich Fig. 3) continua sense cas al corpus; si mai n'apareix un, el destí és `T_ARCH_FEATURES` fins als seus tres casos.*
...
***v25f: primera població sistemàtica i doctrina de capes.*** *La campanya v25f entra 83 arestes noves (total 88, sis famílies — amb el debut de `Horizontal association` al parell EA49↔EA73) i formalitza tres decisions. **Primera, arestes per damunt de grups**: les relacions espacials es registren com a parells encadenats a `T_CONNECTIONS`, mai com a pertinences a `T_GROUPS` — els *clusters* (estrats sencers, alineaments) són **components connexos del graf, derivables a R** (`igraph`), no fets a fossilitzar; això dissol per construcció el problema de la pertinença múltiple (una estructura pot tindre arestes en tantes cadenes com calga, però només un `ID_Group`). `T_GROUPS` queda reservada per a agrupacions genuïnament funcionals-interpretatives. **Segona, ordre de precedència quan un parell apareix en més d'una lectura** (generalitza el criteri «la junta mana sobre el suport»): contacte (junta adossada / superposició / context natural associat) > proximitat sense contacte > estrat compartit; la lectura subsumida és derivable o baixa a `Notes`. **Tercera, usos fixats**: `Shared support` = mateix estrat de suport geològic **independentment de la distància**, amb les cadenes descompostes en parells consecutius seguint l'ordre físic al llarg del front (verificat per coordenades: desnivells consecutius ≤4 m); `Vertical association` cobreix la proximitat vertical **i diagonal** sense contacte; `Associated natural context` = **qualsevol context natural (CAV o NIX) funcionalment associat a qualsevol estructura construïda** — no només el cas paradigmàtic terrassa + nínxol del v18 — i porta cronologia pel principi **el context natural precedeix la construcció** (`Sequential` + `ID_Earlier` = el context natural). I la paternitat (`ID_Parent`) es manté fora del graf: contenció física dins de contextos naturals que són EA de ple dret (CAV/NIX) — precedent EA42, ampliat amb DW-S01-EA76 (diedre), DW-S05-EA02 i DW-S06-EA04.*
...
| — | — | **55** | **v26: reformulada sobre l'expressió derivada — la parella reclama l'esqueixada (qualsevol membre *Fabric*) fora de la finestra `Jambs ∈ {0, 2}` amb `Sys_Portal` Present\*. Mateixa finestra, mateixa protecció, cap camp emmagatzemat.** Formulació històrica: `Jamb_Fabric_Reveal = 1` fora de la finestra (v19, forma de la 49; REVISADA v23): la finestra és `Jambs ∈ {0, 2}` i `Sys_Portal` Present\* — sense obertura, l'esqueixada no té subjecte. Amb el portal Absent solapa l'exempció de la regla 3: totes dues diuen la veritat** |
...
| --- | --- | --- |
| `src/chachapoya_DB.bas` | BuildDB() | Construcció completa sobre una BD EN BLANC: 22 taules, valors per defecte (0 als 20 elements, NULL a la resta; sistemes a Absent), lookups, 25 relacions, **33 consultes**, i l'índex únic `UQ_CONN_PAIR`. Informa del **recompte de consultes creades contra les previstes** |
| `src/chachapoya_Form.bas` | BuildForm() | Després de BuildDB(): **7 subformularis** (F_CONN_IN inclòs), F_STRUCTURES amb 12 pestanyes, combos de dues columnes, captions DAO, gating i validacions de subformulari |
| `tools/migrations/chachapoya_EXPORT_v16.bas` | ExportAll() | **S'executa sobre la BD v16 d'origen.** Escriu 8 CSV inspeccionables i editables. **Cap identificador travessa la frontera**: totes les claus alienes ixen com el valor llegible que apunten |
| `tools/migrations/chachapoya_IMPORT_v17.bas` | ImportAll() | **S'executa sobre una v17 acabada de construir i buida.** Es nega a arrancar si `T_STRUCTURES` ja té files. Resol totes les claus **abans** de `rs.AddNew`, **normalitza les parelles de connexió girant la cronologia amb elles**, en deduplica les col·lisions i informa per fila |
| `tools/patches/chachapoya_PATCH_v17a.bas` | PatchV17a() | **S'executa sobre una v17 AMB DADES.** Afig els cinc camps de notes, reanomena `Notes` → `Doc_Notes` (renom de TableDef: **conserva els valors**), bolca `Outline_Geometry` a `Notes` de cada fila i elimina la columna, refà la meitat rupestre de `L_DEC_TYPE` **repuntant les files abans d'esborrar cap entrada**, i retira la regla 41. Deixa **sense tipus** les files de banda perimetral i les llista a `QRY_22` |
| `tools/diagnostic_v13.bas` | RunDiagnostic() | **Només lectura.** Comprovacions de coherència i de variància sobre el corpus; no modifica cap dada ni cap esquema. Es pot executar tantes vegades com calga |
...
|---|---|---|---|
| Jamb_Fabric | TEXT(25) | Parelles de muntants (v25e): Slab-Slab / Slab-Composite / Slab-Fabric / Composite-Composite / Composite-Fabric / Fabric-Fabric / **Slab-NObs / Composite-NObs / Fabric-NObs** (segon membre no observable) / ND — ordre canònic el més fort primer, sense esquerra/dreta; l'observat primer, NObs sempre segon. **ND (redefinit v25e)** = cap muntant amb resolució *determinable* (no observable o indeterminat); l'evidència positiva de llosa, si n'hi ha, la porta l'element O, mai la parella | Presència de portal (`Sys_Portal Like 'Present*'`); coherència amb l'element O vigilada per la regla R69 (bateria `QRY_16g_Rules_57_69`; **esmena v25e**: la branca d'O absent dispara només amb `Slab-*`/`Composite-*` — afirmen llosa; `Fabric-NObs` no dispara mai — el costat ocult deixa O lliure) |
