...
Const FW  As Long = 9600  ' amplada del formulari
Const LW  As Long = 1900  ' amplada etiqueta
Const CW  As Long = 2500  ' amplada control
Const CH  As Long = 315   ' alçada control
Const LH  As Long = 270   ' alçada etiqueta
Const MT  As Long = 200   ' marge superior pàgina
...
Sub CreateMainForm()
    ...
    ' --- CAPCALERA ---
    f.Section(acHeader).Height = 760
    f.Section(acHeader).BackColor = RGB(26, 60, 107)

    Dim h As Control
    Set h = CreateControl(FRM, acLabel, acHeader, "", "", 120, 90, 7000, 520)
    h.Caption = "FITXA D'ESTRUCTURA  ·  La Petaca & Diablo Wasi (PALP)"
    h.FontSize = 14
    h.FontBold = True
    h.ForeColor = RGB(255, 255, 255)
    h.BackStyle = 0
    h.BorderStyle = 0

    ' --- DETALL ---
    f.Section(acDetail).Height = 6400
    f.Section(acDetail).BackColor = RGB(249, 249, 248)

    ' Pestanya principal
    Dim tc As Control
    Set tc = CreateControl(FRM, acTabCtl, acDetail, "", "", 60, 60, FW - 120, 6260)
    tc.Name = "tabMain"

    ' Pàgines (les dues primeres ja existeixen)
    tc.Pages(0).Name = "pgId":   tc.Pages(0).Caption = "1. Identificació"
    tc.Pages(1).Name = "pgArq":  tc.Pages(1).Caption = "2. Arquitectura"

    Dim pg As Control
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgAcab": pg.Caption = "3. Acabats"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgDec":  pg.Caption = "4. Decoració"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgEst":  pg.Caption = "5. Estat"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgBio":  pg.Caption = "6. Bioarqueologia"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgMat":  pg.Caption = "7. Materials"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgCron": pg.Caption = "8. Cronologia"
    Set pg = CreateControl(FRM, acPage, acDetail, "tabMain")
    pg.Name = "pgSIG":  pg.Caption = "9. SIG i doc."
...
' ── PESTANYA 1: IDENTIFICACIÓ ───────────────────────────────────
Sub FillId(f As String)
    SH f, "pgId", "Identificació i localització", 0
...
Sub FillArq(f As String)
    ...
    PCT f, "pgArq", "Nre. murs construïts:", "N_Murs_Construits", 3, 1
    PCT f, "pgArq", "Llarg (m):",          "Largo_m",           4, 1
    PCT f, "pgArq", "Ample (m):",          "Ancho_m",           5, 1
    PCT f, "pgArq", "Alçada estructura (m):", "Alto_m",         6, 1
    PCT f, "pgArq", "Alt. sobre sòl (m):", "Altura_Aprox_m",   7, 1
    PCT f, "pgArq", "Orientació vano:",    "Orientacio_Vano",   1, 2
...
' ── PESTANYA 4: DECORACIÓ ───────────────────────────────────────
Sub FillDec(f As String)
    SH f, "pgDec", "Baix-relleu", 0
    PCB f, "pgDec", "Nínxol quadrat:",    "Dec_Nicho_Quadrat",  1, 1
    ...
    PCB f, "pgDec", "Geomètrica:",        "PR_Geometrica",      2,  2
    PCB f, "pgDec", "Abstracta:",         "PR_Abstracta",       3,  2
    PCB f, "pgDec", "Escena decapitació:","PR_Escena_Decap",    4,  2
End Sub

' ── PESTANYA 5: ESTAT DE CONSERVACIÓ ───────────────────────────
Sub FillEst(f As String)
    SH f, "pgEst", "Estat de conservació i alteracions", 0
...
Sub FillCron(f As String)
    ...
    PCB f, "pgCron", "Datació C14:",          "C14",             1, 1
    ...
    PCC f, "pgCron", "Mètode càlcul:",        "ID_Metode_Volum",   8, 1
    PCT f, "pgCron", "Notes volum:",          "Volum_Notes",       6, 2
End Sub

' ── PESTANYA 9: SIG I DOCUMENTACIÓ ─────────────────────────────
Sub FillSIG(f As String)
    ...
    PCT f, "pgSIG", "Precisió (m):",     "Coord_Precisio_m", 6, 1
    PCC f, "pgSIG", "Mètode coords:",    "ID_Coord_Metode",  7, 1
    SH  f, "pgSIG", "Documentació digital", 0
