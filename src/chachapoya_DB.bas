...
' ================================================================
'  CHACHAPOYA ARCHAEOLOGICAL DATABASE - COMPLETE BUILD SCRIPT v2
'  La Petaca & Diablo Wasi (Leymebamba, Amazonas, Peru)
'  Author: Esteve Ribera Torro | TFM Arqueologia UA 2024-25
'  Version: v3 - add QRY_13_AX_Pattern_Export for co-occurrence / cluster / I-Moran analysis
'
'  Changes vs v1:
'   - T_STRUCTURES: +10 fields (116 total)
'     * Support detail  (H02): Support_Width_cm, Support_Depth_cm,
'                               Support_Morphology, Support_Modified
'     * Masonry quality (H01/H04): Masonry_Quality, Masonry_Type
'     * Landscape obs.  (QGIS prep): Facade_Orientation, Visibility_Valley
'     * Constructive phases (H03/H04): Construction_Phases, Phase_Evidence
'   - 2 new queries from v2 (12 total):
'     * QRY_11_Masonry_by_Site
'     * QRY_12_Geology_Construction (H02)
'
'  Changes vs v2 (v3):
'   - 1 new query (13 total):
'     * QRY_13_AX_Pattern_Export: net A-X vector export for R
'       (co-occurrence phi/Jaccard, cluster, I de Moran, CA)
'       Uses LEFT JOIN on L_TYPOLOGY to include partial records
'   - QRY_05 and QRY_07 updated with new fields
'
'  Run Sub BuildDB() on a NEW BLANK ACCESS DATABASE
'  Then run chachapoya_02_Form_v2.bas -> Sub BuildForm()
' ================================================================
...
Sub BuildDB()
    ...
    Set db = Nothing
    Dim msg As String
    msg = "DATABASE v2 BUILT SUCCESSFULLY!" & vbCrLf & vbCrLf
    msg = msg & "  18 tables" & vbCrLf
    msg = msg & "  All lookups populated" & vbCrLf
    msg = msg & "  18 relationships" & vbCrLf
    msg = msg & "  13 queries" & vbCrLf
    msg = msg & "  T_STRUCTURES: 116 fields" & vbCrLf & vbCrLf
    msg = msg & "New fields (v2):" & vbCrLf
    msg = msg & "  Support detail (H02): 4 fields" & vbCrLf
    msg = msg & "  Masonry quality (H01/H04): 2 fields" & vbCrLf
    msg = msg & "  Landscape observational: 2 fields" & vbCrLf
    msg = msg & "  Constructive phases (H03/H04): 2 fields" & vbCrLf & vbCrLf
    msg = msg & "Next: run chachapoya_02_Form_v2.bas -> BuildForm()"
...
Private Sub CreateAllTables(db As DAO.Database)
    ...
    ' -- MAIN TABLE: T_STRUCTURES (116 fields) --
    If Not TableExists(db, "T_STRUCTURES") Then
        ...
        ' --- 2b. Support geology detail - v2 (4) | H02: geology as determinant ---
        ...
        ' --- 2c. Masonry quality - v2 (2) | H01/H04: cf. Toyne & Anzellini 2017 ---
        sql = sql & "Masonry_Quality TEXT(20),"
        sql = sql & "Masonry_Type TEXT(30),"
        ' --- 3. Arch. systems N0 - Base (10) ---
        sql = sql & "Base_Level YESNO,"
        sql = sql & "Decorative_Socle YESNO,"
        sql = sql & "Corbelled_Platform YESNO,"
        sql = sql & "Timber_Brackets YESNO,"
        sql = sql & "Timber_Bracket_Count INTEGER,"
        sql = sql & "Transverse_Beams YESNO,"
        sql = sql & "Corbelled_Courses YESNO,"
        sql = sql & "Corbel_Material TEXT(20),"
        sql = sql & "Embedded_Base_Beams YESNO,"
        sql = sql & "Tie_Walls YESNO,"
        ' --- 4. Arch. systems N1 - Access opening (9) ---
        sql = sql & "Access_Opening YESNO,"
        sql = sql & "Sill YESNO,"
        sql = sql & "Jambs YESNO,"
        sql = sql & "Lateral_Walls YESNO,"
        sql = sql & "Rear_Wall YESNO,"
        sql = sql & "Rear_Wall_Built YESNO,"
        sql = sql & "Recessed_Portal YESNO,"
        sql = sql & "Structural_Pilasters YESNO,"
        sql = sql & "Cornice_Material TEXT(20),"
        ' --- 5. Upper zone (6) ---
        sql = sql & "Eave YESNO,"
        sql = sql & "Eave_Beam YESNO,"
        sql = sql & "Eave_Surface YESNO,"
        sql = sql & "Upper_Crown YESNO,"
        sql = sql & "Chamber_Roof YESNO,"
        sql = sql & "Corner_Quoins YESNO,"
        ' --- 6. Finishes (4) ---
        sql = sql & "Plastered YESNO,"
        sql = sql & "Plaster_Color TEXT(20),"
        sql = sql & "Rock_Painting YESNO,"
        sql = sql & "Rock_Paint_Color TEXT(20),"
        ' --- 6b. Landscape & orientation - v2 (2) | observational; QGIS analysis later ---
        sql = sql & "Facade_Orientation TEXT(5),"
        sql = sql & "Visibility_Valley TEXT(10),"
        ' --- 7. Decoration summary booleans (14, detail in T_DECORATIONS) ---
        ...
        ' --- 11b. Constructive phases - v2 (2) | H03: accumulative saturation / H04: operative sequence ---
        ...
        sql = sql & "Notes MEMO)"
        db.Execute sql, dbFailOnError
        Debug.Print "[OK] T_STRUCTURES (116 fields)"
    End If
...
Private Sub PopulateAllLookups(db As DAO.Database)
    ...
    dt(8,0)="Painted band":         dt(8,1)="Horizontal painted band (red/white). Interlevel cornice zone."
...
' ================================================================
'  3. RELATIONSHIPS (18 total - unchanged from v1)
' ================================================================
Private Sub CreateAllRelationships(db As DAO.Database)
    ...
    Debug.Print "-> 18 relationships OK"
End Sub
...
' ================================================================
'  4. QUERIES (12 total)
'     QRY_01 to QRY_10: identical to v1 except QRY_05 and QRY_07
'     QRY_11_Masonry_by_Site:         new (v2)
'     QRY_12_Geology_Construction:    new (v2, H02)
' ================================================================
Private Sub CreateAllQueries(db As DAO.Database)
    ...
    ' QRY_05 - Full flat export for R/SPSS (updated v2: +10 new fields)
    ...
    ' QRY_07 - Spatial export for QGIS (updated v2: +3 new fields)
    ...
    ' QRY_11 - Masonry quality by site and typology (v2 | H01/H04 / cf. Toyne & Anzellini 2017)
    q = "SELECT S.Site_Name, T.Name AS Typology, "
    ...
    ' QRY_12 - Geology-construction relationship (v2 | H02: geology as determinant)
    q = "SELECT S.Site_Name, T.Name AS Typology, "
    ...
    ' QRY_13 - A-X pattern export for R (v3 | co-occurrence, Jaccard cluster, I de Moran, CA)
    ' LEFT JOIN on L_TYPOLOGY: includes structures without typology (partial records)
    ' Covers: H01 (planning), H04 (operative sequence), H05 (shared tradition, sect.5 TFM)
    q = "SELECT E.ID, E.Code, S.Site_Name AS Site, SC.Sector_Name AS Sector, "
    q = q & "T.Name AS Typology, "
    q = q & "E.Coord_E_UTM, E.Coord_N_UTM, E.Altitude_masl, E.ID_Group, "
    q = q & "E.Base_Level, E.Decorative_Socle, E.Corbelled_Platform, "
    q = q & "E.Timber_Brackets, E.Timber_Bracket_Count, E.Corbel_Material, "
    q = q & "E.Transverse_Beams, E.Corbelled_Courses, "
    q = q & "E.Embedded_Base_Beams, E.Tie_Walls, "
    q = q & "E.Access_Opening, E.Sill, E.Jambs, "
    q = q & "E.Lateral_Walls, E.Rear_Wall, E.Rear_Wall_Built, "
    q = q & "E.Recessed_Portal, E.Structural_Pilasters, E.Cornice_Material, "
    q = q & "E.Eave, E.Eave_Beam, E.Eave_Surface, "
    q = q & "E.Upper_Crown, E.Chamber_Roof, E.Corner_Quoins, "
    q = q & "E.Masonry_Quality, E.Masonry_Type, "
    q = q & "E.Support_Morphology, E.Support_Modified "
    q = q & "FROM ((T_STRUCTURES AS E "
    q = q & "INNER JOIN L_SECTORS AS SC ON E.ID_Sector=SC.ID) "
